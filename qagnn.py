...
proxy_server = '10.17.96.240:7890'

os.environ['http_proxy'] = 'http://' + proxy_server
os.environ['https_proxy'] = 'http://' + proxy_server
...
# 定义了一个函数 evaluate_accuracy，用于评估模型在给定数据集上的准确率。
def evaluate_accuracy(eval_set, model):
    n_samples, n_correct = 0, 0

    # 将模型设置为评估模式，这样在评估过程中不会进行梯度计算。
    model.eval()

    # 进入一个 torch.no_grad() 上下文，确保在评估过程中不会进行梯度计算。
    with torch.no_grad():

        # 遍历评估数据集 eval_set 中的每个样本，其中 qids 是问题标识符列表，labels 是标签列表，input_data 包含模型输入所需的所有数据。
        for qids, labels, *input_data in tqdm(eval_set):

            # 使用模型对输入数据进行前向传播，得到预测的逻辑张量 logits。
            logits, _ = model(*input_data)

            # 计算模型预测正确的样本数，并累加到 n_correct 中。通过将 logits 中每个样本的预测类别索引与真实标签 labels 进行比较，并将相等的结果求和，得到预测正确的样本数
...
def train(args):
    ...
    # 这段代码设置了随机数生成器的种子，以确保实验的可重复性。如果可用GPU并且args.cuda设置为True，则还设置了PyTorch的CUDA随机种子。
    ...
    # 这里定义了一些文件路径，如配置文件的路径、模型文件的路径和日志文件的路径，并在日志文件中写入标题行。
    ...
    # 定义了一个函数 compute_loss，用于计算损失。根据参数选择使用交叉熵损失函数或边际排名损失函数。
    ...
    # 初始化一些变量，如全局步数、最佳开发集准确率、最终测试集准确率和总损失，并设置模型为训练模式，冻结编码器部分的参数。
    ...
    # try:
        ...
        for epoch_id in range(args.n_epochs):
            ...
            # 这个循环遍历训练数据集中的每个批次。qids 是问题的标识符，labels 是对应的标签，input_data 包含了模型的输入数据。
            for qids, labels, *input_data in dataset.train():

                # 在每个批次开始时，将优化器的梯度置零，以准备计算新的梯度。
                optimizer.zero_grad()
                # bs 是批次的大小，即标签的数量。
                bs = labels.size(0)

                # 在处理大批次时，将批次分成更小的子批次，每个子批次的大小为args.mini_batch_size，以便在内存限制下进行处理。
                for a in range(0, bs, args.mini_batch_size):
                    b = min(a + args.mini_batch_size, bs)

                    # 根据是否启用混合精度训练，使用不同的方式计算损失。如果启用了混合精度训练，则在自动混合精度上下文中计算损失。
                    if args.fp16:
                    ...
                    #对于每个子批次，根据子批次的大小调整损失。
                    loss = loss * (b - a) / bs

                    # 根据是否启用混合精度训练，使用不同的方式计算损失并进行反向传播。
                    if args.fp16:
                ...
                # 如果设置了梯度裁剪的阈值 args.max_grad_norm，则对梯度进行裁剪，以防止梯度爆炸。
                ...
                # 每训练args.log_interval个批次，打印一次训练信息，并更新全局步数。
            ...
            # 将模型切换为评估模式。在评估模式下，模型的行为会发生变化，例如，Batch Normalization 和 Dropout 层的行为会被固定。
            model.eval()
            # 使用验证集评估模型的性能，并计算验证集的准确率。
            dev_acc = evaluate_accuracy(dataset.dev(), model)
            # 检查是否要保存测试集的预测结果，这个设置是从命令行参数中获取的。
            save_test_preds = args.save_model

            # 如果不需要保存测试集的预测结果，则计算测试集的准确率。如果参数 args.test_statements 为真，则进行测试集的评估，否则将测试准确率设置为0.0。
            if not save_test_preds:
                test_acc = evaluate_accuracy(dataset.test(), model) if args.test_statements else 0.0
            # 如果需要保存测试集的预测结果，则准备测试集数据和相关的变量。
            else:
                ...
                # 打开一个文件以写入测试集的预测结果，并进入一个不计算梯度的上下文。
                with open(preds_path, 'w') as f_preds:
                    with torch.no_grad():

                        # 循环遍历测试集数据，其中 qids 是问题的ID，labels 是对应的标签，*input_data 是其他输入数据，如问题的编码、图结构等。
                        for qids, labels, *input_data in tqdm(eval_set):
                            count += 1

                            # 使用模型对输入数据进行预测，返回预测的概率分布、概念ID、节点类型ID、边的索引和边的类型。
            ...
            # 如果当前的验证准确率优于之前的最佳验证准确率
            if dev_acc >= best_dev_acc:

                # 保存当前模型，并更新最佳验证准确率和对应的测试准确率
            ...
            # 将模型切换为训练模式，以便继续进行下一轮的训练。
            model.train()

            # 记录当前时间，用于计算每个批次的处理时间。
            start_time = time.time()

            # 检查是否满足停止训练的条件，即是否超过了解冻模型的轮数并且当前轮数与最佳验证准确率的轮数之差超过了最大允许的轮数。如果满足条件，则退出训练循环。
...
def eval_detail(args):
    ...
    # 确保传入的参数args中包含了模型加载路径load_model_path，然后将其赋值给变量model_path。
    assert args.load_model_path is not None
    model_path = args.load_model_path

    # 加载实体嵌入的路径列表中的所有嵌入文件，并将它们连接在一起，然后将结果转换为 PyTorch 张量。打印出实体嵌入的数量和维度信息。
    cp_emb = [np.load(path) for path in args.ent_emb_paths]
    cp_emb = torch.tensor(np.concatenate(cp_emb, 1), dtype=torch.float)
    concept_num, concept_dim = cp_emb.size(0), cp_emb.size(1)
    print('| num_concepts: {} |'.format(concept_num))

    # 使用torch.load函数加载模型的参数字典和旧参数（模型的配置）。
    model_state_dict, old_args = torch.load(model_path, map_location=torch.device('cpu'))

    # 根据旧参数（模型配置）创建模型对象model。
    ...
    # 根据当前环境中的 GPU 数量和是否启用 CUDA，选择合适的设备。
    if torch.cuda.device_count() >= 2 and args.cuda:
        device0 = torch.device("cuda:0")
        device1 = torch.device("cuda:1")
    elif torch.cuda.device_count() == 1 and args.cuda:
    ...
    # 将模型的编码器部分移动到 device0，解码器部分移动到 device1，然后将模型切换为评估模式。
    model.encoder.to(device0)
    model.decoder.to(device1)
    model.eval()

    # 加载训练、验证和测试语句，并将它们存储在字典 statement_dic 中。
    statement_dic = {}
    ...
    # 创建数据加载器 dataset，用于加载训练、验证和测试数据。
    dataset = LM_QAGNN_DataLoader(args, args.train_statements, args.train_adj,
                                           args.dev_statements, args.dev_adj,
                                           args.test_statements, args.test_adj,
                                           batch_size=args.batch_size, eval_batch_size=args.eval_batch_size,
    ...
    if not save_test_preds:
        ...
        # 使用 evaluate_accuracy 函数评估测试集的准确率，并将结果赋值给 test_acc。如果 args.test_statements 为真，则评估测试集的准确率，否则将 test_acc 设为 0.0。
        test_acc = evaluate_accuracy(dataset.test(), model) if args.test_statements else 0.0

    # 需要保存测试集的预测结果
    else:

        # 加载测试集数据并初始化变量 total_acc、count、dt 和 preds_path。
        eval_set = dataset.test()
        total_acc = []
        count = 0
        ...
        # 打开文件 preds_path 以写入预测结果，并使用 torch.no_grad() 上下文，确保在评估过程中不会进行梯度计算。然后对测试集中的每个样本进行预测，并将预测结果写入文件中。同时，计算每个样本的准确率并将其添加到 total_acc 中。
        with open(preds_path, 'w') as f_preds:
            with torch.no_grad():
                for qids, labels, *input_data in tqdm(eval_set):
                    count += 1
