...
def eval_detail(args):
    ...
    model = LM_QAGNN(old_args, old_args.encoder, k=old_args.k, n_ntype=4, n_etype=old_args.num_relation,
    ...
    while True:
        ...
        if question.lower() == "exit":
            break
        answer = generate_answer(question, tokenizer, model, device)
        print("Answer: ", answer)


def generate_answer(question, tokenizer, model, device):
